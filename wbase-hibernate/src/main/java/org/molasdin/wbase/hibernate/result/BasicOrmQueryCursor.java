...
import org.molasdin.wbase.hibernate.OrmMatchMode;
...
public class BasicOrmQueryCursor<T> extends CommonOrmCursor<T, Pair<Pair<String,String>, Map<String,Object>>> {
    ...
    private void populateFilters(StrBuilder builder, String from, Map<String, FilteringMode> matchModes){
        if(filters().size() > 0){
            ...
            for(String prop: filters().keySet()){
                ...
                OrmMatchMode matchMode = OrmMatchMode.START;
                if(matchModes.containsKey(prop)){
                    matchMode = OrmMatchMode.fromFilteringMode(matchModes.get(prop));
                }
